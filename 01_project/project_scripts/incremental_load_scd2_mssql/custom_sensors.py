...
class CustomSqlSensor(BaseSensorOperator):
    """
    ...
    def poke(self, context: Any):
        hook = self._get_hook()
        rows_num_xcom = context['ti'].xcom_pull(task_ids=self.xcom_task_id, key=self.xcom_task_id_key)

        self.log.info('Poking: %s (with parameters %s)', self.sql, self.parameters)
        records = hook.get_records(self.sql, self.parameters)

        # count result from sql query
        rows_num_sql = records
        ...
        if self.xcom_task_id and self.xcom_task_id_key:
            if first_cell == rows_num_xcom:
                return True
            else:
                raise AirflowException(f"Xcom and sql query values are not equal -> {rows_num_xcom} != {first_cell}")
